...
def train(network_def,
  ...
  def loss_fn(
    ...
    ve_dist = 0.0
    num_supports = len(support_params)
    for i in range(num_supports):
      local_param = support_params[i]
...
class VESACAgent(sac_agent.SACAgent):
  ...
  def __init__(self,
    ...
    # VE: Support
    self.support_params = [self.target_params for i in range(num_support)]
    self.latest_support = 0
  ...
  def _train_step(self):
    ...
    if self._replay.add_count > self.min_replay_history:
      if self.training_steps % self.update_period == 0:
        ...
        # VE: Add to support
        self.support_params[self.latest_support] = copy.deepcopy(self.target_params)
        self.latest_support = (self.latest_support + 1) % (len(self.support_params))
